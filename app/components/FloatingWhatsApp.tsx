...
export default function FloatingWhatsApp() {
  ...
  const [chatHistory, setChatHistory] = useState([
    { 
      sender: 'support', 
      text: `¡Hola! ¿Cómo podemos ayudarte hoy con ${CONSTANTS.BRAND_NAME}?`, 
      time: getCurrentTime() 
    },
  ]);
  ...
  const phoneNumber = (CONSTANTS.CONTACT.phone || '+34612345678').replace(/[^0-9]/g, '');
  ...
  const SupportProfilePic = () => (
    <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center overflow-hidden flex-shrink-0 border-2 border-white/20">
      <Image
        ...
        alt={`${CONSTANTS.BRAND_NAME} Atención al Cliente`}
        width={40}
        height={40}
    ...
    <div className="whatsapp-global-wrapper">
      ...
      {open && (
        <div 
          ...
          role="dialog"
          aria-label="Ventana de chat en directo de WhatsApp"
        >
          {/* Header */}
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h3 className="font-bold text-sm sm:text-base truncate">
                  {CONSTANTS.BRAND_NAME} Soporte
                </h3>
                <p className="text-[10px] sm:text-xs opacity-90 truncate">
                  Responde normalmente en pocos minutos
                </p>
              </div>
            </div>
            <button
