...
export default function SocialShare({
  title = `${CONSTANTS.BRAND_NAME} - Mejor IPTV Provider España 2026`,
    ...
    return [
      {
        ...
        aria: 'Compartir esta página por WhatsApp',
      },
      {
        name: 'Facebook',
        ...
        aria: 'Compartir esta página por Facebook',
      },
      {
        name: 'X (Twitter)',
        href: `https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedTitle}`,
        icon: Twitter,
        bgColor: 'hover:bg-white hover:text-[#111113]',
        aria: 'Compartir esta página por X',
      },
      {
        name: 'Telegram',
        ...
        aria: 'Compartir esta página por Telegram',
      },
      {
        name: 'LinkedIn',
        ...
        aria: 'Compartir esta página por LinkedIn',
      },
    ];
  }, [url, title]);
  ...
  return (
    <div className="w-full flex justify-center items-center px-4 my-8 relative z-10">
      <div className="w-full max-w-4xl bg-white border-2 border-black/10 rounded-2xl md:rounded-full py-4 px-6 sm:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
        
        {/* Callout Header */}
        <div className="flex items-center justify-center gap-2.5 text-[#111113] shrink-0">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          {shareLinks.map((item) => {
            ...
            return (
              <a
                ...
                className={`w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-black/5 border border-black/10 text-[#111113] flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-95 shadow-sm ${item.bgColor}`}
              >
          ...
          <button
            ...
            type="button"
            aria-label="Copiar enlace de la página al portapapeles"
            className={`flex items-center gap-2 px-4 h-10 sm:h-11 rounded-full text-xs font-black uppercase tracking-wider border transition-all duration-200 active:scale-95 cursor-pointer shadow-sm ${
              copied
                ...
                : 'bg-[#CA1421] border-[#CA1421] text-white hover:bg-[#111113] hover:border-[#111113]'
            }`}
          >
            {copied ? (
              <>
